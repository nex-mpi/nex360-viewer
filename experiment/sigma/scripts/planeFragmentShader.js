...
uniform bool is_sigma; // if true, mpi_a will act as sigma
uniform bool is_npy; // shader will decide correctly between image texture and numpy texture
uniform float sigma_scale; // use to scale sigma from [0,1] top [0,R+]
uniform float z_val; // the plane depth from the camera to plane i (direct view without any rotate)
uniform float plane_depth; // depth from plane i to plane i+1 (next plane)
...
float getPlaneDistance(){
    // visual explanation of how this function work
    // @see https://i.imgur.com/UPyAv2f.png
    float point_depth = length(vCoord - cameraPosition);
    return ((z_val + plane_depth) * (point_depth / z_val)) - point_depth;
}
...
float getAlpha()
{
    ...
    if(is_sigma){
        ...
        float sigma = alphas[alpha_ch];
        if(!is_numpy){
            sigma = sigma * sigma_scale;
        }
        return 1.0 - exp(-sigma * distance);
    }else{
